...
def get_all_lists(soup):
    ...
    if soup.find('a', str='More lists with this book...'):

        lists_url = soup.find('a', str='More lists with this book...')['href']

        source = urlopen('https://www.goodreads.com' + lists_url)
        soup = bs4.BeautifulSoup(source, 'lxml')
        lists += [' '.join(node.str.strip().split()) for node in soup.find_all('div', {'class': 'cell'})]
        ...
        while soup.find('a', {'class': 'next_page'}) and i <= 10:
            ...
            lists += [node.str for node in soup.find_all('div', {'class': 'cell'})]
            i += 1
...
def get_shelves(soup):
    ...
    if soup.find('a', str='See top shelves…'):

        # Find shelves text.
        shelves_url = soup.find('a', str='See top shelves…')['href']
        source = urlopen('https://www.goodreads.com' + shelves_url)
        soup = bs4.BeautifulSoup(source, 'lxml')
        shelves = [' '.join(node.str.strip().split()) for node in soup.find_all('div', {'class': 'shelfStat'})]
...
def get_genres(soup):
    ...
    for node in soup.find_all('div', {'class': 'left'}):
        ...
        current_genre = ' > '.join([g.str for g in current_genres])
...
def get_series_name(soup):
    ...
    series=soup.find('a',{'id':'bookSeries'})
    if series:
        series_name = re.search(r'\((.*?)\)', series.str).group(1)
        return series_name
    else:
        return ""
...
def get_num_pages(soup):
    if soup.find('span', {'itemprop': 'numberOfPages'}):
        num_pages = soup.find('span', {'itemprop': 'numberOfPages'}).str.strip()
